...
test("邪悪な儀式は攻撃表示と守備表示を入れ替える", () => {
  assert.equal(curseOfFiendPosition("attack"), "defense");
  assert.equal(curseOfFiendPosition("defense"), "attack");
});
...
test("ゲート・ガーディアンは三魔神が1体ずつそろった時だけ特殊召喚できる", () => {
  assert.equal(canNormalSummonMonster("pr99-gate-guardian"), false);
  assert.deepEqual(gateGuardianMaterialIndexes(["vol5-kazejin", "vol5-sanga", "vol5-suijin"]), [1, 0, 2]);
  assert.equal(gateGuardianMaterialIndexes(["vol5-sanga", "vol5-kazejin", "vol5-kazejin"]), null);
});

test("儀式素材は手札・フィールドから選んだレベルの合計を計算する", () => {
  assert.equal(ritualMaterialLevelTotal([2, 4]), 6);
  assert.equal(ritualMaterialLevelTotal([3, 1, 2]), 6);
  assert.equal(ritualMaterialLevelTotal([]), 0);
});
