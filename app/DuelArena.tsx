...
type DuelState = {
  ...
  pendingChainBoomerang: PendingChainBoomerang | null;
...
export function DuelArena({
  ...
  function summonToon(handIndex: number, position: Position) {
    ...
    performSummon(handIndex, position, [], true);
  }

  function performSummon(handIndex: number, position: Position, tributeIndexes: number[], special = false) {
    if (!duel) return;
    const id = duel.playerHand[handIndex];
    const card = cardById.get(id);
    ...
    const pharaohTrapId = special ? null : pharaohSummonResponseTrap(nextState.cpuSpellTrap, card, "normal");
    const cpuSummonTrapId = pharaohTrapId ?? (!special && (card.atk ?? 0) >= 1000 ? "vol1-trap-hole" : null);
    const cpuTrapIndex = areTrapEffectsNegated(nextState) || !cpuSummonTrapId ? -1 : nextState.cpuSpellTrap.indexOf(cpuSummonTrapId);
    if (!isGodCard(card.id) && position === "attack" && cpuTrapIndex >= 0 && cpuSummonTrapId) {
      ...
      if (trappedMonster && sevenToolsIndex >= 0 && canActivateSevenTools(nextState.playerLp, nextState.playerSpellTrap)) {
        nextState = {
          ...
          log: appendLog(nextState.log, `CPUが${cardById.get(cpuSummonTrapId)?.name ?? "罠カード"}を発動。盗賊の七つ道具を発動しますか？`),
        };
        setDuel(nextState);
        setPendingTribute(null);
        return;
      ...
      if (trappedMonster && solemnIndex >= 0) {
        nextState = {
          ...
          pendingSolemnJudgment: {
            ...
            cardId: cpuSummonTrapId,
            monsterIndex: nextState.playerField.length - 1,
            playerTributes,
            cpuTributes: returnedTributes,
          },
          log: appendLog(nextState.log, `CPUが${cardById.get(cpuSummonTrapId)?.name ?? "罠カード"}を発動。神の宣告を発動しますか？`),
        };
        setDuel(nextState);
        setPendingTribute(null);
        return;
      ...
      nextState = {
        ...
        cpuGraveyard: [...nextState.cpuGraveyard, cpuSummonTrapId],
        log: appendLog(nextState.log, `CPUが${cardById.get(cpuSummonTrapId)?.name ?? "罠カード"}を発動。${card.name}を破壊。`),
      };
  ...
  function activateTimeBomber() {
    if (!duel || duel.turn !== "player" || duel.phase !== "standby") return;
    const index = duel.playerField.findIndex((zone) => zone.id === "ps-23" && !zone.faceDown);
    if (index < 0) return;
    const effect = timeBomberEffect("ps-23", true, duel.phase, duel.playerField.map((zone) => effectiveAtk(zone, duel, "player")));
    if (!effect) return;
    const destroyed = [...duel.playerField];
    const cpuLp = Math.max(0, duel.cpuLp - effect.damage);
    setDuel({
      ...duel,
      playerField: [],
      playerSpellTrap: discardEquips(duel.playerSpellTrap, destroyed),
      playerGraveyard: [...duel.playerGraveyard, ...graveCards(destroyed)],
      cpuLp,
      result: cpuLp === 0 ? "win" : duel.result,
      log: appendLog(duel.log, `タイム・ボマーの効果を発動。自分のモンスター${effect.destroyCount}体を破壊し、CPUに${effect.damage}ダメージ。`),
    });
  }
  ...
  function useSpell(handIndex: number) {
    ...
    if (card.id === "ps-25") {
      const cost = toonWorldActivationCost(card.id, duel.playerLp);
      if (cost === null || duel.playerSpellTrap.length >= FIELD_LIMIT) return;
      const next = removeHandCard(duel, handIndex);
      setDuel({
        ...next,
        playerLp: next.playerLp - cost,
        playerSpellTrap: [...next.playerSpellTrap, card.id],
        log: appendLog(next.log, "1000LPを払い、トゥーン・ワールドを発動。"),
      });
      return;
    }
    if (card.id === "ps-51") {
      ...
      const next = removeHandCard(duel, handIndex);
      setDuel({ ...next, playerSpellTrap: [...next.playerSpellTrap, card.id], log: appendLog(next.log, "平和の使者を発動。ATK1500以上のモンスターは攻撃できない。") });
      return;
    }
    if (card.id === "ps-09") {
      const playerIndexes = new Set(destroyEquippedMonsterIndexes(duel.playerField));
      const cpuIndexes = new Set(destroyEquippedMonsterIndexes(duel.cpuField));
      if (playerIndexes.size + cpuIndexes.size === 0) return;
      const destroyedPlayer = duel.playerField.filter((_, index) => playerIndexes.has(index));
      const destroyedCpu = duel.cpuField.filter((_, index) => cpuIndexes.has(index));
      const resolved: DuelState = {
        ...
        playerGraveyard: [...duel.playerGraveyard, card.id, ...graveCards(destroyedPlayer)],
        cpuGraveyard: [...duel.cpuGraveyard, ...graveCards(destroyedCpu)],
        log: appendLog(duel.log, `成仏を発動。装備カードを持つモンスター${playerIndexes.size + cpuIndexes.size}体を破壊。`),
      };
      setDuel(applyDeckSearchTriggers(resolved, destroyedPlayer, destroyedCpu));
      return;
    }
  ...
  function beginAcidTrapHole(trapIndex: number) {
    if (!duel || !isPlayerMainPhase || areTrapEffectsNegated(duel) || duel.playerSpellTrap[trapIndex] !== "pr99-acid-trap-hole") return;
    if (!duel.cpuField.some((zone) => zone.faceDown && !isEffectTargetProtected(duel, "cpu", zone))) return;
    setPendingAcidTrapHole(trapIndex);
    setSelectedAttacker(null);
    setSelectedEquip(null);
  }

  function resolveAcidTrapHole(targetIndex: number) {
    if (!duel || pendingAcidTrapHole === null || duel.playerSpellTrap[pendingAcidTrapHole] !== "pr99-acid-trap-hole") return;
    const target = duel.cpuField[targetIndex];
    if (!target || !target.faceDown || isEffectTargetProtected(duel, "cpu", target)) return;
    const targetName = cardById.get(target.id)?.name ?? "モンスター";
    const destroys = (cardById.get(target.id)?.def ?? 0) <= 2000;
    const resolved: DuelState = {
      ...duel,
      cpuField: destroys ? duel.cpuField.filter((_, index) => index !== targetIndex) : duel.cpuField,
      cpuSpellTrap: destroys ? discardEquips(duel.cpuSpellTrap, [target]) : duel.cpuSpellTrap,
      playerSpellTrap: duel.playerSpellTrap.filter((_, index) => index !== pendingAcidTrapHole),
      playerGraveyard: [...duel.playerGraveyard, "pr99-acid-trap-hole"],
      cpuGraveyard: destroys ? [...duel.cpuGraveyard, ...graveCards([target])] : duel.cpuGraveyard,
      log: appendLog(duel.log, `硫酸のたまった落とし穴を発動。${targetName}を確認し、${destroys ? "DEFが2000以下のため破壊した。" : "DEFが2000を超えるため裏側守備表示に戻した。"}`),
    };
    setDuel(destroys ? applyDeckSearchTriggers(resolved, [], [target]) : resolved);
    setPendingAcidTrapHole(null);
  }
  ...
  function reviveMonster(graveSide: Side, graveIndex: number, position: Position) {
    ...
    if (spellId === "ca-37") {
      ...
      const cpuChoices = duel.cpuGraveyard
        .map((id, index) => ({ id, index, card: cardById.get(id) }))
        .filter(({ card }) => card?.cardType === "monster" && !card.fusion)
        .sort((a, b) => (b.card?.atk ?? 0) - (a.card?.atk ?? 0));
      const cpuChoice = cpuChoices[0];
      const playerMonster = playerTaken ? cardById.get(playerTaken.cardId) : null;
      const cpuTaken = cpuChoice ? takeGraveyardCard(duel.cpuGraveyard, cpuChoice.index) : null;
      if (!playerTaken || playerMonster?.cardType !== "monster" || !cpuTaken) return;
      ...
      setDuel({
        ...
        playerField: [...duel.playerField, makeSetZone(playerTaken.cardId, "player")],
        cpuField: [...duel.cpuField, makeSetZone(cpuTaken.cardId, "cpu")],
  ...
  function equipSpell(fieldIndex: number, owner: Side = "player") {
    ...
    if (spell.id === "ps-08") {
      ...
      const summoned: ZoneCard = { id: "ps-05", position: "attack", faceDown: false, attacked: false, equipped: [], summonedTurn: duel.turnNumber, faceUpTurn: duel.turnNumber, positionChanged: false };
      setDuel({
        ...removeHandCard(duel, selectedEquip),
        playerDeck: duel.playerDeck.filter((_, index) => index !== wallShadowIndex),
        playerField: [...duel.playerField.filter((_, index) => index !== fieldIndex), summoned],
        playerGraveyard: [...duel.playerGraveyard, ...graveCards([zone]), "ps-08"],
        playerSpellTrap: discardEquips(duel.playerSpellTrap, [zone]),
        log: appendLog(duel.log, "迷宮変化と迷宮壁－ラビリンス・ウォール－をリリースし、ウォール・シャドウを特殊召喚。"),
      });
      setSelectedEquip(null);
      return;
    }
  ...
  function activateCurseOfAnubisTrap(trapIndex: number) {
    ...
    if (trapId === "ca-09") {
      ...
      setDuel({
        ...duel,
        playerLp: duel.playerLp + gain,
        playerSpellTrap: duel.playerSpellTrap.filter((_, index) => index !== trapIndex),
        playerGraveyard: [...duel.playerGraveyard, trapId],
        log: appendLog(duel.log, `ホーリー・エルフの祝福を発動。モンスター${monsterCount}体につき300、合計${gain}LP回復。`),
      });
      return;
    }
    if (trapId === "ca-30") {
      const playerField = duel.playerField.map((zone) => ({ ...zone, faceDown: false, faceUpTurn: zone.faceDown ? duel.turnNumber : zone.faceUpTurn }));
      const cpuField = duel.cpuField.map((zone) => ({ ...zone, faceDown: false, faceUpTurn: zone.faceDown ? duel.turnNumber : zone.faceUpTurn }));
      const effectCount = [...playerField, ...cpuField].filter((zone) => cardById.get(zone.id)?.effect).length;
      const damage = ceasefireDamage(effectCount);
      const cpuLp = Math.max(0, duel.cpuLp - damage);
      setDuel({
        ...duel,
        playerField,
  ...
  function resolvePlayerAttack(attackerIndex: number, defenderIndex: number | null) {
    ...
    if (mirrorForceIndex < 0) {
      ...
      const chainBoomerangIndex = areTrapEffectsNegated(duel) || !attacker || isGodCard(attacker.id)
        ? -1
        : duel.cpuSpellTrap.indexOf("pr99-kunai-chain");
      if (chainBoomerangIndex >= 0) {
        const equipIndex = strongestAttackIndex(duel.cpuField.map((zone) => effectiveAtk(zone, duel, "cpu")));
        const canEquipChain = equipIndex !== null && !duel.cpuField[equipIndex]?.faceDown;
        const payment = duelAttackPayment(duel, "player", attacker);
        if (!payment) return;
        const milled = duel.playerDeck.slice(0, payment.millCount);
        const targetName = canEquipChain ? cardById.get(duel.cpuField[equipIndex!].id)?.name : null;
        setDuel({
          ...duel,
          playerLp: duel.playerLp - payment.lifeCost,
          playerDeck: duel.playerDeck.slice(payment.millCount),
          playerGraveyard: [...duel.playerGraveyard, ...milled],
          playerField: duel.playerField.map((zone, index) => index === attackerIndex
            ? { ...zone, position: "defense", attacked: true, positionChanged: true }
            : zone),
          cpuField: canEquipChain
            ? duel.cpuField.map((zone, index) => index === equipIndex ? { ...zone, equipped: [...zone.equipped, "pr99-kunai-chain"] } : zone)
            : duel.cpuField,
          cpuSpellTrap: canEquipChain
            ? duel.cpuSpellTrap
            : duel.cpuSpellTrap.filter((_, index) => index !== chainBoomerangIndex),
          cpuGraveyard: canEquipChain ? duel.cpuGraveyard : [...duel.cpuGraveyard, "pr99-kunai-chain"],
          log: appendLog(duel.log, `CPUが鎖付きブーメランを発動。攻撃モンスターを守備表示にした。${targetName ? `${targetName}に装備し、ATKを500アップ。` : ""}`),
        });
        return;
      }
    ...
    const attacker = duel.playerField[attackerIndex];
    const payment = attacker ? duelAttackPayment(duel, "player", attacker) : { lifeCost: 0, millCount: 0 };
    if (!payment) return;
    const milled = duel.playerDeck.slice(0, payment.millCount);
  ...
  function respondToSevenTools(activate: boolean) {
    ...
    const counteredTrapId = duel.cpuSpellTrap[pending.trapIndex] ?? "vol1-trap-hole";
    const counteredTrapName = cardById.get(counteredTrapId)?.name ?? "罠カード";
    let resolved: DuelState;
    if (activate) {
      resolved = {
        ...duel,
        ...
        cpuGraveyard: [...duel.cpuGraveyard, counteredTrapId],
        log: appendLog(duel.log, `盗賊の七つ道具を発動。1000LPを払い、CPUの${counteredTrapName}を無効にして破壊した。`),
      };
      ...
      resolved = {
        ...
        cpuGraveyard: [...duel.cpuGraveyard, counteredTrapId],
        log: appendLog(duel.log, `盗賊の七つ道具を発動せず、${cardById.get(pending.monsterId)?.name ?? "モンスター"}が${counteredTrapName}で破壊された。`),
      };
  ...
  function respondToSolemnJudgment(activate: boolean) {
    ...
    if (!activate && pending.monsterIndex !== undefined) {
      ...
      if ((cardById.get(pending.cardId)?.atk ?? 0) >= 1000 && trapIndex >= 0) {
        setDuel({
          ...resumed,
          pendingTrapResponse: { trapIndex, trapId: "vol1-trap-hole", monsterIndex: pending.monsterIndex, monsterId: pending.cardId },
        });
        return;
      }
    }
  ...
  function respondToTrap(activate: boolean) {
    ...
    if (activate) {
      if (pending.trapId === "ca-06") {
        const resolved = chainDestructionResult(pending.monsterId, monster?.atk ?? 0, resumed.cpuHand, resumed.cpuDeck);
        if (!resolved) return;
        resumed = {
          ...resumed,
          cpuHand: resolved.hand,
          cpuDeck: shuffle(resolved.deck),
          playerSpellTrap: resumed.playerSpellTrap.filter((_, index) => index !== pending.trapIndex),
          cpuGraveyard: [...resumed.cpuGraveyard, ...resolved.destroyed],
          playerGraveyard: [...resumed.playerGraveyard, pending.trapId],
          log: appendLog(resumed.log, `連鎖破壊を発動。CPUの手札・デッキから${monster?.name ?? "同名カード"}を${resolved.destroyed.length}枚破壊。召喚されたモンスターはフィールドに残る。`),
        };
        const finalState = finishCpuTurn(resumed);
        beginCpuPlayback(resumed, finalState, marker);
        return;
      ...
      resumed = {
        ...
        playerGraveyard: [...resumed.playerGraveyard, pending.trapId],
        log: appendLog(resumed.log, `${cardById.get(pending.trapId)?.name ?? "罠カード"}を発動。${monster?.name ?? "モンスター"}を破壊。`),
      };
      resumed = applyDeckSearchTriggers(resumed, [], trappedMonster ? [trappedMonster] : []);
    } else {
      resumed = { ...resumed, log: appendLog(resumed.log, `${cardById.get(pending.trapId)?.name ?? "罠カード"}を発動しませんでした。`) };
    }
    const finalState = finishCpuTurn(resumed);
    beginCpuPlayback(resumed, finalState, marker);
  }
  ...
  function respondToMirrorForce(activate: boolean) {
    ...
    const chainBoomerangPrompt = prepareChainBoomerang(resumed, pending.attackerIndex, pending.defenderIndex);
    if (chainBoomerangPrompt) {
      beginCpuPlayback(resumed, chainBoomerangPrompt, marker);
      return;
    }
  ...
  function respondToChainBoomerang(mode: "decline" | "stop" | "equip" | "both", equipIndex?: number) {
    if (!duel?.pendingChainBoomerang) return;
    const pending = duel.pendingChainBoomerang;
    const marker = "鎖付きブーメランの発動確認が終了。";
    const shouldStop = mode === "stop" || mode === "both";
    const shouldEquip = mode === "equip" || mode === "both";
    const equipTarget = equipIndex === undefined ? null : duel.playerField[equipIndex];
    if (shouldEquip && (!equipTarget || equipTarget.faceDown)) return;
    let resumed: DuelState = {
      ...duel,
      pendingChainBoomerang: null,
      log: appendLog(duel.log, marker),
    ...
    if (mode === "decline") {
      resumed = { ...resumed, log: appendLog(resumed.log, "鎖付きブーメランを発動しませんでした。") };
    } else if (resumed.playerSpellTrap[pending.trapIndex] === "pr99-kunai-chain") {
      resumed = {
        ...resumed,
        cpuField: shouldStop
          ? resumed.cpuField.map((zone, index) => index === pending.attackerIndex
            ? { ...zone, position: "defense", attacked: true, positionChanged: true }
            : zone)
          : resumed.cpuField,
        playerField: shouldEquip
          ? resumed.playerField.map((zone, index) => index === equipIndex
            ? { ...zone, equipped: [...zone.equipped, "pr99-kunai-chain"] }
            : zone)
          : resumed.playerField,
        playerSpellTrap: shouldEquip
          ? resumed.playerSpellTrap
          : resumed.playerSpellTrap.filter((_, index) => index !== pending.trapIndex),
        playerGraveyard: shouldEquip
          ? resumed.playerGraveyard
          : [...resumed.playerGraveyard, "pr99-kunai-chain"],
        log: appendLog(
          resumed.log,
          `鎖付きブーメランを発動。${shouldStop ? "攻撃モンスターを守備表示にした。" : ""}${shouldEquip ? `${cardById.get(equipTarget!.id)?.name ?? "自分モンスター"}に装備し、ATKを500アップ。` : ""}`,
        ),
      };
    }
    if (shouldStop) {
    ...
    const defender = pending.defenderIndex === null ? null : resumed.playerField[pending.defenderIndex];
    if (pending.defenderIndex !== null && defender && !defender.faceDown && isGuardianMonster(defender.id) && !defender.guardianEffectUsed) {
      const finalState: DuelState = { ...resumed, pendingGuardianResponse: { attackerIndex: pending.attackerIndex, defenderIndex: pending.defenderIndex, guardianId: defender.id } };
      beginCpuPlayback(resumed, finalState, marker);
      return;
    }
    const kuribohPrompt = prepareKuribohResponse(resumed, pending.attackerIndex, pending.defenderIndex);
    resumed = kuribohPrompt ?? resolveBattle(resumed, "cpu", pending.attackerIndex, pending.defenderIndex);
    const finalState = resumed.pendingKuribohResponse || resumed.pendingWabokuResponse ? resumed : finishCpuTurn(resumed, true);
    beginCpuPlayback(duel, finalState, marker);
  }
  ...
  return (
    <section className={`duel-screen${activeFeedback ? ` action-${activeFeedback.kind}` : ""}`}>
            ...
            <button className="cpu-next" onClick={advanceCpuPlayback}>
              {cpuPlayback.finalState.pendingTrapResponse
                    ...
                    : cpuPlayback.finalState.pendingMirrorForce
                      ? "ミラーフォースの発動確認へ"
                    : cpuPlayback.finalState.pendingChainBoomerang
                      ? "鎖付きブーメランの効果選択へ"
      ...
      {!cpuPlayback && duel.pendingTrapResponse && (
        <div className="trap-response">
          <div>
            ...
            <h2>{cardById.get(duel.pendingTrapResponse.trapId)?.name ?? "罠カード"}を発動しますか？</h2>
            <p>
              CPUが
      ...
      {!cpuPlayback && duel.pendingChainBoomerang && (
        <div className="trap-response">
          <div>
            <p className="section-label">ATTACK RESPONSE</p>
            <h2>鎖付きブーメランを発動しますか？</h2>
            <p>攻撃を止める効果、装備してATKを500上げる効果、または両方を選べます。</p>
            <div className="spell-target-list">
              <button className="activate-trap" onClick={() => respondToChainBoomerang("stop")}>攻撃を止める</button>
              {duel.playerField.map((zone, index) => zone.faceDown ? null : (
                <article key={`${zone.id}-${index}`}>
                  <strong>{cardById.get(zone.id)?.name ?? "自分モンスター"}</strong>
                  <button onClick={() => respondToChainBoomerang("both", index)}>攻撃を止めて装備</button>
                  <button onClick={() => respondToChainBoomerang("equip", index)}>装備だけ行う</button>
                </article>
              ))}
            </div>
            <button onClick={() => respondToChainBoomerang("decline")}>発動しない</button>
          </div>
        </div>
      )}
      ...
      {pendingAcidTrapHole !== null && (
        <div className="card-overlay spell-target-overlay">
          <div className="graveyard-panel spell-target-panel">
            ...
            <h2>硫酸のたまった落とし穴</h2>
            <p>確認するCPUの裏側守備表示モンスターを選んでください。DEF2000以下なら破壊します。</p>
            <div className="spell-target-list">
              {duel.cpuField.map((zone, index) => !zone.faceDown || isEffectTargetProtected(duel, "cpu", zone) ? null : (
                <button key={`${zone.id}-${index}`} onClick={() => resolveAcidTrapHole(index)}>
                  <strong>裏側守備表示モンスター</strong>
                  <small>選択すると確認して処理します</small>
                </button>
              ))}
            </div>
            <button className="secondary" onClick={() => setPendingAcidTrapHole(null)}>キャンセル</button>
          </div>
        </div>
      )}
      {pendingCrushVirus !== null && (
      ...
      {pendingTribute && (
        <div className="tribute-picker">
          ...
          <button
            ...
            onClick={() => performSummon(pendingTribute.handIndex, pendingTribute.position, pendingTribute.selected, pendingTribute.special)}
          >
            選択したモンスターを生け贄にする
          </button>
                  ...
                  <>
                    ...
                    <button
                      disabled={
                        ...
                        || (card.id === "ca-37" && (
                          duel.playerField.length >= FIELD_LIMIT
                          || duel.cpuField.length >= FIELD_LIMIT
                          || isLightOfInterventionActive(duel)
...
function resolveParasiteDraw(state: DuelState, side: Side, drawnId: string): DuelState {
  if (drawnId !== "ca-03") return state;
  ...
  if (state[fieldKey].length >= FIELD_LIMIT) {
    return { ...state, log: appendLog(state.log, "寄生虫パラサイドをドローしたが、モンスターゾーンに空きがなく特殊召喚できなかった。") };
  }
  const life = Math.max(0, state[lifeKey] - 1000);
  return {
    ...state,
    [handKey]: removeCardCopies(state[handKey], drawnId, 1),
    ...
    [lifeKey]: life,
    result: life === 0 ? (side === "player" ? "lose" : "win") : state.result,
    log: appendLog(state.log, `${side === "player" ? "あなた" : "CPU"}が寄生虫パラサイドをドロー。表側守備表示で特殊召喚され、1000ダメージ。`),
  } as DuelState;
}
...
function continueCpuTurnAfterSpells(initial: DuelState): DuelState {
  ...
  if (!state.cpuSpellTrap.includes("ps-25") && toonWorldIndex >= 0 && toonWorldActivationCost("ps-25", state.cpuLp) !== null && state.cpuSpellTrap.length < FIELD_LIMIT) {
    state = {
      ...
      cpuSpellTrap: [...state.cpuSpellTrap, "ps-25"],
  ...
  const toonChoice = state.cpuSpellTrap.includes("ps-25")
    ...
    : undefined;
  if (toonChoice) {
    const tributeTotal = toonSummonTributeCount(toonChoice.card.id) ?? 0;
    const tributeIndexes = lowestAttackIndexes(state.cpuField, tributeTotal);
    const tributes = state.cpuField.filter((_, index) => tributeIndexes.includes(index));
    state = {
      ...state,
      cpuHand: state.cpuHand.filter((_, index) => index !== toonChoice.index),
      cpuField: [
        ...state.cpuField.filter((_, index) => !tributeIndexes.includes(index)),
        { id: toonChoice.card.id, position: "attack", faceDown: false, attacked: true, equipped: [], summonedTurn: state.turnNumber, faceUpTurn: state.turnNumber, positionChanged: false },
      ],
      cpuSpellTrap: discardEquips(state.cpuSpellTrap, tributes),
      cpuGraveyard: [...state.cpuGraveyard, ...graveCards(tributes)],
      log: appendLog(state.log, `CPUが${toonChoice.card.name}を特殊召喚。`),
    };
    state = applyDeckSearchTriggers(state, [], tributes);
  }
  ...
  if (summonChoice) {
    ...
    const pharaohTrapId = pharaohSummonResponseTrap(state.playerSpellTrap, summonChoice.card, "normal");
    const chainDestructionId = (summonChoice.card.atk ?? 0) <= 2000 && state.playerSpellTrap.includes("ca-06") ? "ca-06" : null;
    const summonTrapId = pharaohTrapId ?? chainDestructionId ?? ((summonChoice.card.atk ?? 0) >= 1000 ? "vol1-trap-hole" : null);
    const trapIndex = areTrapEffectsNegated(state) || !summonTrapId ? -1 : state.playerSpellTrap.indexOf(summonTrapId);
    if (!isGodCard(summonChoice.card.id) && !defensive && trapIndex >= 0 && summonTrapId) {
      return {
        ...state,
        pendingTrapResponse: {
          trapIndex,
...
function applyMessengerStandby(state: DuelState, side: Side): DuelState {
  ...
  const count = state[spellKey].filter((id) => id === "ps-51").length;
  if (count === 0) return state;
  const cost = messengerOfPeaceStandbyCost(count);
  ...
  const strongestOwn = Math.max(0, ...ownField.map((zone) => effectiveAtk(zone, state, side)));
  const strongestOpponent = Math.max(0, ...opponentField.map((zone) => effectiveAtk(zone, state, side === "player" ? "cpu" : "player")));
  const keep = side === "player"
    ? state[lpKey] > cost
    : shouldCpuKeepMessengerOfPeace(state[lpKey], count, strongestOwn, strongestOpponent);
  if (!keep) {
    return {
      ...state,
      [spellKey]: state[spellKey].filter((id) => id !== "ps-51"),
      [graveKey]: [...state[graveKey], ...Array(count).fill("ps-51")],
      log: appendLog(state.log, `${side === "player" ? "平和の使者" : "CPUの平和の使者"}の維持コストを払わず破壊。`),
    } as DuelState;
  }
  return {
    ...state,
    [lpKey]: state[lpKey] - cost,
    log: appendLog(state.log, `${side === "player" ? "平和の使者" : "CPUの平和の使者"}の維持コスト${cost}LPを支払った。`),
  } as DuelState;
}
...
function finishCpuTurn(initial: DuelState, resumeBattle = false): DuelState {
  ...
  if (state.playerSwordsTurns.length > 0) {
    ...
    for (let index = state.cpuField.length - 1; index >= 0 && state.phase === "battle" && !state.result && !state.pendingFlipTarget && !state.pendingMultiTarget && !state.pendingDeckReorder && !state.pendingDeckSearch && !state.pendingGuardianResponse && !state.pendingMirrorForce && !state.pendingChainBoomerang && !state.pendingReverseTrap && !state.pendingBattleStatTrap && !state.pendingKuribohResponse && !state.pendingWabokuResponse; index -= 1) {
      ...
      if (state.playerField.length === 0 || toonAttack?.directAttack) {
        ...
        const chainBoomerangPrompt = prepareChainBoomerang(state, index, null);
        if (chainBoomerangPrompt) {
      ...
      if (targetIndex !== null) {
        ...
        const chainBoomerangPrompt = prepareChainBoomerang(state, index, targetIndex);
        if (chainBoomerangPrompt) {
          state = chainBoomerangPrompt;
          continue;
        }
  ...
  if (state.result || state.pendingFlipTarget || state.pendingMultiTarget || state.pendingDeckReorder || state.pendingDeckSearch || state.pendingGuardianResponse || state.pendingMirrorForce || state.pendingChainBoomerang || state.pendingReverseTrap || state.pendingBattleStatTrap || state.pendingKuribohResponse || state.pendingWabokuResponse) return state;
...
function prepareChainBoomerang(state: DuelState, attackerIndex: number, defenderIndex: number | null): DuelState | null {
  if (areTrapEffectsNegated(state) || isGodCard(state.cpuField[attackerIndex]?.id ?? "")) return null;
  const trapIndex = state.playerSpellTrap.indexOf("pr99-kunai-chain");
  if (trapIndex < 0) return null;
  return {
    ...state,
    pendingChainBoomerang: { trapIndex, attackerIndex, defenderIndex },
    log: appendLog(state.log, "CPUが攻撃を宣言。鎖付きブーメランの効果を選んでください。"),
  };
}
...
function duelAttackPayment(state: DuelState, side: Side, monster: ZoneCard) {
  ...
  if (toonSummonTributeCount(monster.id) === null) return payment;
  const toon = toonAttackDeclaration(monster.id, monster.summonedTurn, state.turnNumber, ownLp - payment.lifeCost, ownSpellTraps.includes("ps-25"), opponentField.filter((zone) => toonSummonTributeCount(zone.id) !== null).length);
  return toon ? { ...payment, lifeCost: payment.lifeCost + toon.lifeCost } : null;
}
...
function resolveBattle(state: DuelState, attackerSide: Side, attackerIndex: number, defenderIndex: number | null, guardianEffect = false, preventPlayerBattleDamage = false): DuelState {
  ...
  const payment = duelAttackPayment(state, attackerSide, attackerZone);
...
function applyCardInspectionStandby(state: DuelState, standbySide: Side): DuelState {
  ...
  const targetHand = standbySide === "player" ? state.playerHand : state.cpuHand;
  const inspectorLp = inspectorSide === "player" ? state.playerLp : state.cpuLp;
  const usableCopies = Math.min(copies, Math.max(0, Math.ceil((inspectorLp - 500) / 500)), targetHand.length);
  if (usableCopies === 0) return state;
  const revealed = shuffle([...targetHand]).slice(0, usableCopies).map((id) => cardById.get(id)?.name ?? "カード");
  const cost = usableCopies * 500;
  return {
    ...state,
...
function resolveFlipEffect(state: DuelState, owner: Side, monsterId: string): DuelState {
  ...
  if (effect === "parasite-deck") {
    ...
    const withoutSource = ownerField.filter((zone) => zone !== source);
    const ownerGraveyard = owner === "player" ? state.playerGraveyard : state.cpuGraveyard;
    const graveIndex = ownerGraveyard.lastIndexOf(monsterId);
    const cleanedGraveyard = graveIndex < 0 ? ownerGraveyard : ownerGraveyard.filter((_, index) => index !== graveIndex);
    return owner === "player"
      ? {
          ...state,
          playerField: withoutSource,
          playerSpellTrap: source ? discardEquips(state.playerSpellTrap, [source]) : state.playerSpellTrap,
          playerGraveyard: [...cleanedGraveyard, ...(source?.equipped ?? [])],
          cpuDeck: shuffle([...state.cpuDeck, monsterId]),
          log: appendLog(state.log, "あなたの寄生虫パラサイドがリバース。CPUのデッキに表向きで混ぜた。"),
        }
      : {
          ...state,
          cpuField: withoutSource,
          cpuSpellTrap: source ? discardEquips(state.cpuSpellTrap, [source]) : state.cpuSpellTrap,
          cpuGraveyard: [...cleanedGraveyard, ...(source?.equipped ?? [])],
          playerDeck: shuffle([...state.playerDeck, monsterId]),
          log: appendLog(state.log, "CPUの寄生虫パラサイドがリバース。あなたのデッキに表向きで混ぜた。"),
        };
  }
  if (effect === "cyber-jar") {
    const playerRevealed = state.playerDeck.slice(0, 5).map((id) => cardById.get(id)).filter((card): card is Card => Boolean(card));
    const cpuRevealed = state.cpuDeck.slice(0, 5).map((id) => cardById.get(id)).filter((card): card is Card => Boolean(card));
    const playerResult = cyberJarReveal(playerRevealed, FIELD_LIMIT);
    const cpuResult = cyberJarReveal(cpuRevealed, FIELD_LIMIT);
    const toZone = (card: Card, side: Side): ZoneCard => ({
      id: card.id,
      position: (card.atk ?? 0) >= (card.def ?? 0) ? "attack" : "defense",
      faceDown: (card.atk ?? 0) < (card.def ?? 0),
      attacked: side === "cpu",
      equipped: [],
      summonedTurn: state.turnNumber,
      positionChanged: false,
    ...
    const resolved: DuelState = {
      ...state,
      playerDeck: state.playerDeck.slice(playerResult.revealedCount),
      cpuDeck: state.cpuDeck.slice(cpuResult.revealedCount),
      playerField: playerResult.summonCards.map((card) => toZone(card as Card, "player")),
      cpuField: cpuResult.summonCards.map((card) => toZone(card as Card, "cpu")),
      playerHand: [...state.playerHand, ...playerResult.handCards.map((card) => card.id)],
      cpuHand: [...state.cpuHand, ...cpuResult.handCards.map((card) => card.id)],
      playerSpellTrap: discardEquips(state.playerSpellTrap, destroyedPlayer),
      cpuSpellTrap: discardEquips(state.cpuSpellTrap, destroyedCpu),
      playerGraveyard: [...state.playerGraveyard, ...graveCards(destroyedPlayer)],
      cpuGraveyard: [...state.cpuGraveyard, ...graveCards(destroyedCpu)],
...
function trapDescription(id: string) {
  ...
  if (id === "pr99-kunai-chain") return "相手の攻撃モンスターを守備表示にする、または自分の表側モンスターに装備してATKを500アップ（両方も選択可能）";
  if (id === "pr99-acid-trap-hole") return "相手の裏側守備表示モンスター1体を確認し、DEF2000以下なら破壊。超えていれば裏側守備表示に戻す";
  const card = cardById.get(id);
  return card ? cardDescription(card) : "";
}
...
function removeNewGraveCards(previous: string[], next: string[]) {
  ...
  previous.forEach((id) => oldCounts.set(id, (oldCounts.get(id) ?? 0) + 1));
  let removed = 0;
  const kept = next.filter((id) => {
    const count = oldCounts.get(id) ?? 0;
    if (count > 0) {
      oldCounts.set(id, count - 1);
      return true;
    }
    removed += 1;
    return false;
...
function moveHornFromGraveToDeck(graveyard: string[], deck: string[], count: number) {
  let remaining = count;
  const kept = graveyard.filter((id) => {
    if (id !== "ps-03" || remaining <= 0) return true;
    remaining -= 1;
    return false;
  });
  const returned = count - remaining;
  return { graveyard: kept, deck: [...Array(returned).fill("ps-03"), ...deck], returned };
}

function applyPharaohContinuousRules(previous: DuelState, initial: DuelState): DuelState {
  let next = initial;
  let log = next.log;
  ...
  for (const side of ["player", "cpu"] as const) {
    ...
    const spellKey = side === "player" ? "playerSpellTrap" : "cpuSpellTrap";
    const graveKey = side === "player" ? "playerGraveyard" : "cpuGraveyard";
    if (previous[spellKey].includes("ps-25") && !next[spellKey].includes("ps-25")) {
      const destroyed = next[fieldKey].filter((zone) => toonSummonTributeCount(zone.id) !== null);
      if (destroyed.length > 0) {
        next = {
          ...next,
          [fieldKey]: next[fieldKey].filter((zone) => toonSummonTributeCount(zone.id) === null),
          [spellKey]: discardEquips(next[spellKey], destroyed),
          [graveKey]: [...next[graveKey], ...graveCards(destroyed)],
        } as DuelState;
        log = appendLog(log, `${side === "player" ? "トゥーン・ワールド" : "CPUのトゥーン・ワールド"}が破壊され、トゥーン${destroyed.length}体も破壊された。`);
      }
    }
  }

  const faceUpZones = [...next.playerField, ...next.cpuField].filter((zone) => !zone.faceDown);
  if (faceUpZones.some((zone) => zone.id === "ps-44")) {
    const duplicateNames = new Set(kotodamaDuplicateIndexes(faceUpZones.map((zone) => cardById.get(zone.id)?.name ?? zone.id)).map((index) => cardById.get(faceUpZones[index].id)?.name ?? faceUpZones[index].id));
    if (duplicateNames.size > 0) {
      const destroyedPlayer = next.playerField.filter((zone) => !zone.faceDown && duplicateNames.has(cardById.get(zone.id)?.name ?? zone.id));
      const destroyedCpu = next.cpuField.filter((zone) => !zone.faceDown && duplicateNames.has(cardById.get(zone.id)?.name ?? zone.id));
      next = {
        ...next,
        playerField: next.playerField.filter((zone) => zone.faceDown || !duplicateNames.has(cardById.get(zone.id)?.name ?? zone.id)),
        cpuField: next.cpuField.filter((zone) => zone.faceDown || !duplicateNames.has(cardById.get(zone.id)?.name ?? zone.id)),
        playerSpellTrap: discardEquips(next.playerSpellTrap, destroyedPlayer),
        cpuSpellTrap: discardEquips(next.cpuSpellTrap, destroyedCpu),
        playerGraveyard: [...next.playerGraveyard, ...graveCards(destroyedPlayer)],
        cpuGraveyard: [...next.cpuGraveyard, ...graveCards(destroyedCpu)],
      };
      log = appendLog(log, `コトダマの効果で同名モンスター${destroyedPlayer.length + destroyedCpu.length}体を破壊。`);
    }
  }

  const playerHornLoss = Math.max(0, previous.playerSpellTrap.filter((id) => id === "ps-03").length - next.playerSpellTrap.filter((id) => id === "ps-03").length);
  const cpuHornLoss = Math.max(0, previous.cpuSpellTrap.filter((id) => id === "ps-03").length - next.cpuSpellTrap.filter((id) => id === "ps-03").length);
  const playerHorn = moveHornFromGraveToDeck(next.playerGraveyard, next.playerDeck, playerHornLoss);
  const cpuHorn = moveHornFromGraveToDeck(next.cpuGraveyard, next.cpuDeck, cpuHornLoss);
  next = { ...next, playerGraveyard: playerHorn.graveyard, playerDeck: playerHorn.deck, cpuGraveyard: cpuHorn.graveyard, cpuDeck: cpuHorn.deck };
  if (playerHorn.returned + cpuHorn.returned > 0) log = appendLog(log, "一角獣のホーンが持ち主のデッキの一番上へ戻った。");

  const banisherActive = banisherRedirectsToExile(faceUpZones.map((zone) => zone.id));
  if (banisherActive) {
    const playerRedirect = removeNewGraveCards(previous.playerGraveyard, next.playerGraveyard);
    const cpuRedirect = removeNewGraveCards(previous.cpuGraveyard, next.cpuGraveyard);
    next = { ...next, playerGraveyard: playerRedirect.kept, cpuGraveyard: cpuRedirect.kept };
    const removed = playerRedirect.removed + cpuRedirect.removed;
    if (removed > 0) log = appendLog(log, `光の追放者の効果で墓地へ送られるカード${removed}枚を除外。`);
  }
  return { ...next, log };
}
...
function applyAutomaticCurseResponses(previous: DuelState, initial: DuelState): DuelState {
  ...
  if (!areTrapEffectsNegated(next) && !next.result) {
    ...
    const cpuTriggers = cpuCopies * sharedRideTriggerCount(previous.log, initial.log, "player", previous.playerDeck.length - initial.playerDeck.length, initial.turn);
    if (playerTriggers > 0) {
      const amount = playerTriggers * 2;
      next = next.playerDeck.length < amount
        ? { ...next, result: "lose", log: appendLog(next.log, `便乗の効果で${amount}枚ドローできず、デッキ切れで敗北。`) }
        : { ...next, playerDeck: next.playerDeck.slice(amount), playerHand: [...next.playerHand, ...next.playerDeck.slice(0, amount)], log: appendLog(next.log, `便乗の効果で${amount}枚ドロー。`) };
    }
    if (cpuTriggers > 0 && !next.result) {
      const amount = cpuTriggers * 2;
